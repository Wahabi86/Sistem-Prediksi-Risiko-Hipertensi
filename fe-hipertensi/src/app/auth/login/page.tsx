...
export default function LoginPage() {
  ...
  // Fungsi untuk menangani ketikan user
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.msg || "Email atau kata sandi salah");
  ...
  // --- STYLE INPUT DIKEMBALIKAN KE VERSI LAMA ANDA ---
  // (Transparan bg-white/20, teks putih, border white/30)
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className={`${inputStyle} pr-12`} // pr-12 untuk memberi ruang tombol mata
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-cyan-100 lg:text-gray-400 hover:text-white lg:hover:text-cyan-600 transition-colors"
                >
